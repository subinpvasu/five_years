...
Project     : BTW Dive
Author      : Subin
Title      : Monthly Invoice Banner
Description : Monthly Invoice document banner allows to print those docs.
-->
<script>
...
var diverid=document.getElementById("diverid").value;
	...
	loadAJAX().onreadystatechange=function()
	  ...
	  {
		  xmlhttp.open("POST",base_url+"index.php/customer/ajax_bowaft_search/"+str,true);
	  }
	  else if(document.getElementById("findpdf").value==101)
		  {
		  xmlhttp.open("POST",base_url+"index.php/customer/ajax_list_invoice_search/"+frm+"/"+to+"/0/40/"+str,true);//----------------------------------
		  }
		else if(document.getElementById("findpdf").value==105)
		  {
		  xmlhttp.open("POST",base_url+"index.php/customer/ajax_diver_commi_search/"+diverid+"/"+frm+"/"+to+"/0/40/"+str,true);//----------------------------------
		  }
	  else
	  {
	xmlhttp.open("POST",base_url+"index.php/customer/ajax_balance_search/"+str,true);
	  }
	xmlhttp.send();
	}
}
function searchandfindpdf()
{
	var str = document.getElementById("balanceout").innerHTML;
	    ...
	    if(str!='')
	    {
	    	if(document.getElementById("findpdf").value==100)
	    	{
	    		$.ajax({url:base_url+"index.php/customer/searched_pdf/1",
	    			type:"post",
	    			 data: {
	    				data:str
	    			 },
	    	...
	    	{
	    		$.ajax({url:base_url+"index.php/customer/searched_pdf/2",
	    			type:"post",
	    			 data: {
	    				data:str
	    			 },
	    	...
	    	{
	   $.ajax({url:base_url+"index.php/customer/searched_pdf/",
			type:"post",
			 data: {
				data:str
			 },
