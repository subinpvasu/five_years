...
Author      : Subin
Title      : Send Invoices
Description : Allows to send invoices to customers
-->
<script>
function send_invoice(str)
{
	$("#worktoday").html('<img width="100px" height="100px" src="<?php echo base_url() ?>img/loading_gif.gif" style="position:relative;top:0px;left:0px;">');
		$.ajax({url:base_url+"index.php/customer/displaySendInvoice/",
			type:"post",
			 data: {
				 	one:str
...
function delete_invoice(str)
{
		...
		$.ajax({url:base_url+"index.php/customer/deleteInvoice/",
			type:"post",
			 data: {
				 	one:str
...
function void_invoice(str)
{
		...
		$.ajax({url:base_url+"index.php/customer/voidInvoice/",
			type:"post",
			 data: {
				 	one:str
...
$(document).ready(function(){
	$("#send").click(function(){
		if($(":checkbox").is(":checked"))
		{
		    ...
		    chkId =  chkId.slice(0,-1);
		    $.ajax({url:base_url+"index.php/customer/sendInvoicesEmail/",
				type:"post",
				 data: {
					 	address:chkId
...
<h2 style="width:100%;text-align: center;">List of Invoices Not Sent</h2>
