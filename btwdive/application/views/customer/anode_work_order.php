...
Project     : BTW Dive
Author      : Subin
Title      : Anode Workorder
Description : Anode work order displays in this page.
-->
<style>
#hdmk {
	position:fixed;
		  ...
		  if (xmlhttp.readyState==4 && xmlhttp.status==200)
		    {
		      ...
		      if(xmlhttp.responseText=='Y')
		      {
		    	  setTimeout(function(){alert("Work Order Completed.");realClose();},3000);
...
realClose();
	...
	xmlhttp.open("POST",base_url+"index.php/customer/anode_pdf/"+str,true);
	xmlhttp.send();*/
...
else//add
{
    ...
    $.ajax({url:base_url+"index.php/customer/updateWorkOrder/"+str,
		type:"post",
		 data: {
			 workpk:pkwork,
			 wonom:wonum,
...
foreach ( $anodes as $anode ) :
    $changed_date=$anode->CHANGED_DATE;
if(($changed_date=='01/01/1900')||($changed_date=='12/31/1969'))
{
    $changed_date="";
}
    echo '
...
if(count($pkw)>1  && $status==0)
{
      foreach($totalanodes as $t)
        {
            $type_description = '';
         $t_changed_date=$t->ADDFIELD1;
         if(($t_changed_date=='01/01/1900')||($t_changed_date=='12/31/1969'))
{
    $t_changed_date="";
}
if($t->ANODE_TYPE=='')
...
<td>
<textarea id="type'.$t->PK_VESSEL_ANODE.'">'.$type_description.'</textarea>
<!--<input type="text" name="" id="type'.$t->PK_VESSEL_ANODE.'"  value="'.$t->ANODE_TYPE.'" class="textbox" style="width:auto;"/>-->
</td>
<td><input type="text" name="" id="date'.$t->PK_VESSEL_ANODE.'" value="'.$t_changed_date.'" class="textbox date" style="width:auto;"/></td>
