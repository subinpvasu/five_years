...
Project     : BTW Dive
Author      : Subin
Title      : Daily Work Order
Description : Dispaly sthe whole open work orders.
-->
...
function dailyWorkOrder(sbn,smn,sort)
{
	$("#worktoday").html('<img width="100px" height="100px" src="<?php echo base_url() ?>img/loading_gif.gif" style="position:relative;top:0px;left:0px;">');
	 ...
	 $(".same").val(1);
	$.ajax({url:base_url+"index.php/customer/displayDailyWorkOrder/",
		type:"post",
		 data: {
			 	one:sbn,
			 	two:smn,
...
$(".photos").css('display','none');
$(".same").val(1);
	$.ajax({url:base_url+"index.php/customer/displayDailyWorkOrderDate/",
		type:"post",
		 data: {
			 	one:sbn,
			 	two:smn,
...
function printDocument()
{
	...
	$.ajax({url:base_url+"index.php/customer/printDocumentNow/",
		type:"post",
		 data: {
			 	data:datum
		 },
