...
<script type="text/javascript">
    function delete_commission_details()
    {
        ...
        $.ajax({url: base_url+"index.php/customer/deleteDiverCommission/",
            type: "post",
            data: {
...
//Modify Commission Details
    function updateCommission()
    {
        ...
        $.ajax({url: base_url+"index.php/customer/updateDiverCommission/",
            type: "post",
            data: {
    ...
    function popup(wo)
    {
          $.ajax({url: base_url+"index.php/customer/displayCommissionDetails/",
                type: "post",
                data: {
                    one: wo
...
<script>
    ...
    function updateList()
    {
        ...
        $.ajax({url: base_url+"index.php/customer/displayDiverCommission/",
            type: "post",
            data: {
                one: $("#diver").val(),
                from: $("#fromdate").val(),
...
/*
 * Process payment function on click
 * @returns {undefined}
 */
$(document).ready(function() {
        $("#process_pay").click(function(ev) {
            ...
            if($("#diver").val()=='')
            {
                alert('Please select a diver');
                ev.preventDefault();
            }
            else if($("#deduct").val()=="")
            {
                ev.preventDefault();
                alert('Deduction Amount Empty');
            }
            else
            {
            process_payment();
...
$(document).ready(function() {
    ...
    {
        $.ajax({url: base_url+"index.php/customer/processPayment/",
            type: "post",
            data: {
    ...
    /*
     * change total on adding deduction amount
     */
   function findNewtotal()
   {
        var totalCommision = $("#ctotal").val();
        var deduction = $("#deduct").val();
        var finalTotal=parseFloat(totalCommision-deduction).toFixed(2);
                //  newtotal = newtotal.toFixed(2);
                $("#newTotal").val(finalTotal);
   }
   /*
    * Number
    */
    ...
    <table>
        ...
        ?>
            ...
            <td><input type="text" class="textbox"
                       style="width: auto" id="deduct" onkeypress="return isNumberKey(event)" onkeyup="findNewtotal()"/></td>
        </tr>
        <tr>
            <td></td>
            <td>New Total</td>
