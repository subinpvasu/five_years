...
Description : Manage Credit Payment Details of customers.
-->
...
// }
    ...
    function paymentSave()
    {
        ...
        else
        {
            ...
            $.ajax({url: base_url+"index.php/customer/saveCreditPayment/",
                type: "post",
                data: {
                    repair:$("#repair").val(),
    ...
    function sendNotificationCreditCard(str)
    {
        var r = confirm("Are you sure you want to send the request?");
        if(r)
        {
       	 $.ajax({url: base_url+"index.php/customer/sendNotificationCreditCard/"+str,
           	 success:function(result)
           	 {
alert(result);
           	 }
...
//alert(id);
    ...
    {
        $.ajax({url: base_url+"index.php/customer/deleteCreditPayment/",
            type: "post",
            data: {
                ledger_id: pk_ledger,
            },
...
#maskbyme
{
    ...
    background-image: url("<?php echo base_url() ?>img/loading_gif.gif");
...
<?php endforeach; ?>
    ...
    <tr>
        <td colspan="3" style="text-align: center;"><button class="btn" id="savebtn" onClick="paymentSave()">Save</button>
        <?php if(preg_match("/CREDIT/i",$credit_card)){?>
        <button class="btn" style="width:auto;" id="" onClick="sendNotificationCreditCard(<?php echo $cid; ?>)">Request CC info</button>
        <?php }?>
