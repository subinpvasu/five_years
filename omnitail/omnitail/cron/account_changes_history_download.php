            ...
            foreach ($page->entries as $adGroup) {
                if ($campaign_type == 'SHOPPING') {
                    ...
                    $sql = "INSERT INTO adgroup_data(customerid, campaignid, adgroupid,crid,campaign_name,adgroup_name,bid,crname,added) VALUES
												 ('" . $_SESSION['account_number'] . "','" . $adGroup->campaignId . "','" . $adGroup->id . "','" . $crid . "','" . mysqli_real_escape_string($conn, $adGroup->campaignName) . "','" . mysqli_real_escape_string($conn, $adGroup->name) . "','" . $bid . "','" . $crname . "',NOW())";
                    $conn->query($sql);
                }
            ...
            foreach ($page->entries as $adGroup) {
                if ($campaign_type == 'SHOPPING') {
                   ...
                   /*  $adGroupCriterionService = $user->GetService('AdGroupCriterionService', ADWORDS_VERSION);
                    ...
                    $crname = '';
                    $crid = 0;
                    $sql = "UPDATE adgroup_data SET campaign_name='" . mysqli_real_escape_string($conn, $adGroup->campaignName) . "',adgroup_name='" . mysqli_real_escape_string($conn, $adGroup->name) . "',bid='" . $bid . "',crname='" . $crname . "' WHERE campaignid=$adGroup->campaignId AND customerid=" . $_SESSION['account_number'] . " AND adgroupid=".$adGroup->id." AND crid=".$crid;
                    $conn->query($sql);
                }
...
function update_shopping_adgroup(AdWordsUser $user,$adgroupid)
{
    ...
    do {
        ...
        {
            $mysql = "UPDATE criterion_data set crname='".$crname."', crbid='".$bid."',updated=NOW() WHERE customerid=" . $_SESSION['account_number'] . " AND campaignid=".get_campaign_id($adgroupid)." AND adgroupid=".$adgroupid." AND criterionid=".$crid;
            //  echo $mysql;
            $conn->query($mysql);
        }
        else {
...
function get_adgroupid_bycampaignid($campaignid)
{
    global $conn;
    $adgroup = array();
...
try {
 ...
 $name = $manage==1?mysqli_real_escape_string($conn, $a->name):mysqli_real_escape_string($conn, $a->descriptiveName);
    ...
    $sqla = "SELECT *,a.account_number AS account FROM prospect_credentials p INNER JOIN account_details a ON p.account_number=a.mccid WHERE account_status=1 ";// AND a.account_number=5493971629 
