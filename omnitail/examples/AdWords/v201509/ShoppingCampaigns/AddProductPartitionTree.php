...
//  $helper->createUnit($root, new ProductCustomAttribute('CUSTOM_ATTRIBUTE_0', '40 Love Courture Backpacks'), 100000);
//  $otherBrand = 
//          $helper->createSubdivision($root, new ProductCustomAttribute());
  ...
  $helper->createUnit($otherBrand,
      new ProductBiddingCategory('BIDDING_CATEGORY_L1', '513743549703189181'), 750000);
  $helper->createUnit($otherBrand,
...
function displayTree($node, $children, $level = 0) {
  ...
  printf("%sid: %s, type: %s, value: %s<br>", str_repeat("  ", $level),
      $node->id, $type, $value);
...
$user->SetClientCustomerId('718-172-2645');
  ...
  //https://developers.google.com/adwords/api/docs/reference/v201601/AdGroupCriterionService.ProductPartition
} catch (Exception $e) {
  printf("An error has occurred:<br> %s<br>", $e->getMessage());
}
