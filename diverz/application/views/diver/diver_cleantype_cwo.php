...
if(!($(this).prop("checked")))
 {
            		 ...
            		 {
         			    dir=dir+"|"+$(this).attr("id");
	}
                 });

            	 $.ajax({url:"<?php echo base_url();?>index.php/customer/temporaryAdjustment/",
          			type:"post",
          			 data: {
           			    excluded:tmp,
           			    included:dir
             ...
             {
                 alert("To complete this work order, one services should be done");
             }//alert(str);
         }
   ...
   echo 'Half Clean';
       ...
       // echo $c->WORK_DESCRIPTION;
        ...
        ?> </td>
            ...
            <?php
           $c->WORK_VALUE == 0 ? print '<td style="background-color:#262626;text-align: center;width: 25%"><input type="checkbox" class="chkr"  id="' . $c->PK_WO_PARTS . '" value="' . $c->WORK_VALUE . '"/></td>' : print
      '<td style="background-color:#262626;text-align: center;width: 25%"><input type="checkbox" checked id="' . $c->PK_WO_PARTS . '" value="' . $c->WORK_VALUE . '" name="'.$c->PK_WORK.'"/></td>';
            ?>
        </tr>        <?php
}
endforeach;
