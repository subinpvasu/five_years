...
realClose();
	...
	xmlhttp.open("POST","/btwdive/index.php/customer/cleaning_pdf/"+str,true);
	xmlhttp.send();*/
...
function changeDateCleaning(dates,days)
{
	...
	xmlhttp.open("POST","/btwdive/index.php/customer/changeCleaningDates/"+dates+"/"+days,true);
	xmlhttp.send();
...
function correctDateChecker(dates)
{
	...
	$.ajax({url:"/btwdive/index.php/customer/checkCorrectDate/",
		type:"post",
		 data: {
			    date:dates
		 },
...
{
	if($("#rfstatus").val()==0){
	$(".refreshed").css('display','block');
	var nd = $("#datelast").val();
	$("#scheduledate").val(nd);
	changewoNumber();
	$(".chkr").click();
	$(".refreshed:checkbox").click();
...
$c->WORK_VALUE == 0 ? print '<td><input type="checkbox" class="chkr"  id="process' . $c->PK_WO_PARTS . '" value="' . $c->WORK_VALUE . '"/></td>' : print 
    '<td><input type="checkbox" checked id="process' . $c->PK_WO_PARTS . '" value="' . $c->WORK_VALUE . '" name="'.$c->PK_WORK.'"/></td>';
...
<td><input type="checkbox" class="chkr"  id="process'.$t->PK_VESSEL_SERVICE.'" value="1" name="'.$t->PK_VESSEL_SERVICE.'"/></td>
