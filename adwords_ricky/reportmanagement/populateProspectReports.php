...
//$file = "/var/www/html/app/reportmanagement/setup/config.ini";
...
$prospectDailyReportsSql = "INSERT INTO `".Constants::$PROSPECT_DAILY_REPORT_TABLE_NAME."`(`user_id_fk`,`user_name`, `client_name`, `report_status`, `Budget`, `ppc_visitors_last_month`, `ppc_visitors_current_month`, `percent_at_this_point_last_month`, `cpc_last_month`, `cpc_current_month`, `change_in_cpc`, `ppc_spend_last_month`, `ppc_spend_current_month`, `remaining_budget`, `remaining_budget_at_ppc_spend`, `daily_budget`, `avg_daily_spends_mtd`, `plus_or_minus_daily_budget_available`, `yesterday_spends`, `adwords_conversions_last_month`, `adwords_conversions_current_month`, `conversions_at_current_rate`, `percent_on_last_month_at_current_rate`, `ppc_cpa_last_month`, `ppc_cpa_current_month`, `change_in_cpa`, `last_updated`, `client_id`,`cpa_target`,`budget_cap`) VALUES ";
$prospectSummeryReportsSql = "INSERT INTO `".Constants::$PROSPECT_SUMMERY_REPORT_TABLE_NAME."`(`user_id`,`user_name`, `client_name`, `Top_5_biggest_increse_in_visitors`, `Biggest_Increases_in_CPC`, `Biggest_underspends`, `Worst_5_with_biggest_decrease_in_visitors`, `Best_Decreases_in_CPC`, `biggest_Overspends`, `Biggest_Gain_in_Expected_Conversions`, `Biggest_Change_in_CPA`, `Worst_drop_in_expected_conversions`, `Best_Change_in_CPA`, `percent_Visitors_to_Last_Month`, `CPC_Diff`, `Remaining_Spends`, `percent_Conversions_to_Last_Month`, `CPC_Change`, `CPA_Change`) VALUES ";
...
$getUserQuery = "SELECT * FROM `".Constants::$ADWORD_USER_TABLE_NAME."`";
...
while($user = mysqli_fetch_array($user_table->returnObject)){
    if($user['ad_user_report_link'] != '' && $user['ad_user_type']==4) {
        ...
        foreach ($worksheets as $value) {
            $ws = $value->getTitle();
            if($value->getTitle() == Constants::$SHEET_TITLE1 ){
                //&& $ownerName==Constants::$MASTER_NAME
                ...
                foreach ($entries as $entry){
                    ...
                    //Checking heading
                    if($headerSettingStatus == Constants::$HEADER_SETTING_NOT_STARTED){
                        if($entry->getContent() == Constants::$MANAGER_NAME){
                            $headerSettingStatus = Constants::$HEADER_SETTING_ONGOING;
                            continue;
                        }
                    }

                    //Checking heading
                    if($headerSettingStatus == Constants::$HEADER_SETTING_ONGOING){
                        if($entry->getColumn()>=Constants::$MAX_COLUMNS){
                            $headerSettingStatus = Constants::$HEADER_SETTING_FINISHED;
                        }
                        continue;
                    }


                    if($headerSettingStatus ==Constants::$HEADER_SETTING_FINISHED){
                        //Processing first column
                        if($dataRowStatus ==Constants::$INT_NO && $entry->getColumn()== Constants::$FIRST){
                            $dataRowStatus = Constants::$INT_YES;
                            ...
                            $prospectDailyReportsSql .= "(".$userId.",'".addslashes($entry->getContent())."'";
                        }else if($dataRowStatus == Constants::$INT_YES){

                            //checks whether current cell and previous cell belongs to the same row.
                            if($entry->getRow()==$previousRow && $entry->getColumn()<=Constants::$MAX_COLUMNS){

                                //if previous column is a null column, then a null entry will be filled before current value
                                if(($entry->getColumn()-1)==$previousColumn){
                                    $prospectDailyReportsSql.=",'".addslashes($entry->getContent())."'";
                                }else{
                                    $colDiff = $entry->getColumn()-$previousColumn;
                                    $emptyColumns = $helper->fillEmptyColumns($colDiff);
                                    $prospectDailyReportsSql.=$emptyColumns.",'".addslashes($entry->getContent())."'";
                                }
                            //checks whether last cell of last row is mepty
                            //then a null value is added before completing the set
                            }else if($entry->getColumn()>Constants::$MAX_COLUMNS){
                                continue;
                            }else if($previousColumn!=Constants::$MAX_COLUMNS){
                                $userName = strtolower($entry->getContent()).Constants::$DEFAULT_EMAIL_TAIL;
                                $user = $helper->getUser($userName);
                                if($user->responseCode==Constants::$RESPONSE_CODE_OK){
                                    $userId = $user->returnObject->userId;
                                ...
                                $colDiff = Constants::$MAX_COLUMNS-$previousColumn;
                                $emptyColumns = $helper->fillEmptyColumns($colDiff+1);
                                $prospectDailyReportsSql.=$emptyColumns."),(".$userId.",'".addslashes($entry->getContent())."'";
                            }else{
                                $userName = strtolower($entry->getContent()).Constants::$DEFAULT_EMAIL_TAIL;
                                $user = $helper->getUser($userName);
                                if($user->responseCode==Constants::$RESPONSE_CODE_OK){
                                ...
                                //adds a closing bracket to show it is the end of the set
                                $prospectDailyReportsSql.="),(".$userId.",'".addslashes($entry->getContent())."'";
                            }
                ...
                if($previousColumn!=Constants::$MAX_COLUMNS){
                    $colDiff = Constants::$MAX_COLUMNS-$previousColumn;
                    $emptyColumns = $helper->fillEmptyColumns($colDiff+1);
                    $prospectDailyReportsSql.=$emptyColumns.");";
                }else{
                    $prospectDailyReportsSql.=");";
                }
    ...
    echo $ex->getMessage();
    mail('bisjo.vbridge@gmail.com', 'Cron finished with error at '.date('d-m-y H:i:s'), $ex->getMessage());
    exit();
}
...
$truncate = $dbObj->exeuteQuery("TRUNCATE TABLE ".Constants::$PROSPECT_DAILY_REPORT_TABLE_NAME);
...
echo "<br>";
echo $prospectDailyReportsSql;echo "<br>";
$response = $dbObj->exeuteQuery($prospectDailyReportsSql);
if($response->responseCode==Constants::$RESPONSE_CODE_OK){
    echo $response->responseMessage;
}else{
    echo $response->responseMessage;
...
$truncate = $dbObj->exeuteQuery("TRUNCATE TABLE ".Constants::$PROSPECT_SUMMERY_REPORT_TABLE_NAME);
if($truncate->responseCode == Constants::$RESPONSE_CODE_OK){
    echo $truncate->responseMessage;
}else{
    echo $truncate->responseMessage;
...
$response = $dbObj->exeuteQuery($prospectSummeryReportsSql.$summeryReportsSqlValues);
if($response->responseCode==Constants::$RESPONSE_CODE_OK){
    echo $response->responseMessage;
}else{
    echo $response->responseMessage;
...
if(strpos($r, 'Failed')!=false){
    mail('shijo.vbridge@gmail.com', 'Cron finished with error at '.date('d-m-y H:i:s'), $r.$prospectDailyReportsSql.$prospectSummeryReportsSql.$summeryReportsSqlValues);
}

//echo $prospectDailyReportsSql;
//echo $prospectSummeryReportsSql.$summeryReportsSqlValues;