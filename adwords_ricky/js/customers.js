...
$(document).ready(function() {
	...
	$(document).on("click", ".En_Dis", function(){	
		...
		$.post('servicefiles/customers_to_original.php',{ids:ids,flag:flag},function(data){
			loadAccountDetails(page_num,'');
...
function loadAccountDetails(page_num,search_term){
	...
	$.post('servicefiles/customers.php',{page:page_num,search_term:search_term},function(data){
...
function gotoAccounts(customer){
	
	$.post('servicefiles/gotoAccounts.php',{customer:customer},function(data){
			if(data['msg']==1){ window.location.replace("account_details.php"); }
		},'json');
