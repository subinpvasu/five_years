 ...
 * 
 * @namespace UserhomeBackendCalendar
 */
var UserhomeBackendCalendar = {
    // :: CONSTANTS
    ...
    initialize: function(defaultEventHandlers) {
        ...
        $('#calendar').fullCalendar({
            ...
            'height': UserhomeBackendCalendar.getCalendarHeight(),
            'editable': true,
            'firstDay': 1, // Monday
            'slotMinutes': 30,
            'snapMinutes': 15,
            ...
            'windowResize': UserhomeBackendCalendar.calendarWindowResize,
            'viewDisplay': UserhomeBackendCalendar.calendarViewDisplay,
            'dayClick': UserhomeBackendCalendar.calendarDayClick,
            'eventClick': UserhomeBackendCalendar.calendarEventClick,
            'eventResize': UserhomeBackendCalendar.calendarEventResize,
            'eventDrop': UserhomeBackendCalendar.calendarEventDrop,
            'eventAfterAllRender': function(view) {
                UserhomeBackendCalendar.convertTitlesToHtml();
            }   
        });
        
        // Trigger once to set the proper footer position after calendar 
        // initialization.
        UserhomeBackendCalendar.calendarWindowResize();
        ...
        //                + 'type="' + UserhomeBackendCalendar.FILTER_TYPE_PROVIDER + '" '
        ...
        //                'type="' + UserhomeBackendCalendar.FILTER_TYPE_SERVICE + '">' +
        //                service['name'] + '</option>';
        //    });
        //    optgroupHtml += '</optgroup>';
        //    $('#select-filter-item').append(optgroupHtml);
        ...
        if (GlobalVariables.availableAdminUsers.length > 0) {
            ...
            $.each(GlobalVariables.availableAdminUsers, function(index, adminuser) {
                optgroupHtml += '<option value="' + adminuser['id'] + '" ' +
                        'type="' + UserhomeBackendCalendar.FILTER_TYPE_ADMINUSER + '">' +
                        adminuser['first_name'] + '</option>';
            });
            optgroupHtml += '</optgroup>';
            $('#select-filter-item').append(optgroupHtml);
        }
        ...
        if (defaultEventHandlers === true) {
            UserhomeBackendCalendar.bindEventHandlers();
        ...
        if (GlobalVariables.editAppointment != null) {
            ...
            UserhomeBackendCalendar.resetAppointmentDialog();
    ...
    bindEventHandlers: function() {
        ...
        $('#select-filter-item').change(function() { 
            UserhomeBackendCalendar.refreshCalendarAppointments(
                    $('#calendar'),
            ...
            if ($('#select-filter-item option:selected').attr('type') 
                    === UserhomeBackendCalendar.FILTER_TYPE_SERVICE) {
        ...
        $(document).on('click', '.edit-popover', function() {
            ...
            if (UserhomeBackendCalendar.lastFocusedEventData.data.is_unavailable == false) {
                var appointment = UserhomeBackendCalendar.lastFocusedEventData.data;
                ...
                var unavailable = UserhomeBackendCalendar.lastFocusedEventData.data;
                
                // Replace string date values with actual date objects.
                unavailable.start_datetime = GeneralFunctions.clone(UserhomeBackendCalendar.lastFocusedEventData.start);
                unavailable.end_datetime = GeneralFunctions.clone(UserhomeBackendCalendar.lastFocusedEventData.end);
                
                $dialog = $('#manage-unavailable');
                UserhomeBackendCalendar.resetUnavailableDialog();
            ...
            if (UserhomeBackendCalendar.lastFocusedEventData.data.is_unavailable == false) {
                ...
                messageButtons['OK'] = function() {
                    ...
                    var postData = { 
                        ...
                        'appointment_id' : UserhomeBackendCalendar.lastFocusedEventData.data['id'],
                ...
                var postData = { 
                    'csrfToken': GlobalVariables.csrfToken,
                    'unavailable_id' : UserhomeBackendCalendar.lastFocusedEventData.data.id
                };
        ...
        $('#manage-appointment #save-appointment').click(function() {
            ...
            if (!UserhomeBackendCalendar.validateAppointmentForm()) {
            ...
            UserhomeBackendCalendar.saveAppointment(appointment, customer,
        ...
        $('#manage-unavailable #save-unavailable').click(function() {
            ...
            UserhomeBackendCalendar.saveUnavailable(unavailable, successCallback, errorCallback);
        });
            ...
            if ($('#enable-sync').hasClass('enabled') === false) {
                ...
                $.each(GlobalVariables.availableProviders, function(index, provider) {
                    if (provider['id'] == $('#select-filter-item').val()) {
                        ...
                        UserhomeBackendCalendar.disableProviderSync(provider['id']);
    ...
    refreshCalendarAppointments: function($calendar, recordId, filterType, startDate, endDate) {
        ...
        $.post(postUrl, postData, function(response) {
            ...
            if (filterType === UserhomeBackendCalendar.FILTER_TYPE_ADMINUSER && calendarView !== 'month') {
                $.each(GlobalVariables.availableAdminUsers, function(index, adminuser) {
    ...
    calendarEventResize: function(event, dayDelta, minuteDelta, revertFunc, 
        ...
        if (event.data.is_unavailable == false) {
            ...
            UserhomeBackendCalendar.saveAppointment(appointment, undefined,
            ...
            UserhomeBackendCalendar.saveUnavailable(unavailable, successCallback, undefined);
        }
    },
    ...
    calendarWindowResize: function(view) {
        $('#calendar').fullCalendar('option', 'height', 
                UserhomeBackendCalendar.getCalendarHeight());
    },
    ...
    calendarEventClick: function(event, jsEvent, view) {
        ...
        UserhomeBackendCalendar.lastFocusedEventData = event;
