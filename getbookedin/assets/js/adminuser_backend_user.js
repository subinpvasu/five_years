 ...
 * 
 * @namespace AdminUserBackendUser
 */
var AdminUserBackendUser = {
    ...
    initialize: function(defaultEventHandlers) {
        ...
        $('#filter-adminuser .results').jScrollPane();
        //$('#filter-secretaries .results').jScrollPane();
        
        // Instanciate default helper object (admin).
        AdminUserBackendUser.helper = new AdminUserHelper();
        //AdminUserBackendUser.helper.resetForm();
        AdminUserBackendUser.helper.filter(GlobalVariables.user.email,GlobalVariables.user.id);
        
        AdminUserBackendUser.wp = new WorkingPlan();
        AdminUserBackendUser.wp.bindEventHandlers();
        ...
        */
        //
        if (GlobalVariables.adminuser.settings.duration) {
            $('#duration')
                .find('option[value="' + GlobalVariables.adminuser.settings.duration + '"]').prop('selected', true);
            //console.log($('#select-filter-item').val());
            $('#duration').prop('disabled', true);
        }
                
        $.each(GlobalVariables.adminuser, function(index, user) {
           var html = '<label class="checkbox"><input type="checkbox" data-id="' + user.id + '" />'
                   + user.first_name + ' ' + user.last_name + '</label>';
            $('#secretary-providers').append(html);
        });
        //$('#secretary-adminusers').jScrollPane({ mouseWheelSpeed: 70 });
        ...
        // Bind event handlers.
        if (defaultEventHandlers) AdminUserBackendUser.bindEventHandlers();
    },
    ...
    bindEventHandlers: function() {
        ...
        $('.tab').click(function() {
            ...
            if ($(this).hasClass('admins-tab')) { // display admins tab
                $('#admins').show();
                AdminUserBackendUser.helper = new AdminsHelper();
            } else if ($(this).hasClass('adminuser-tab')) { // display providers tab
                $('#adminuser').show();
                ...
                $('#secretaries').show();
                AdminUserBackendUser.helper = new SecretariesHelper();
        ...
        $('#adminuser-email').focusout(function() {
            var $input = $(this);
        ...
        AdminUserHelper.prototype.bindEventHandlers();
