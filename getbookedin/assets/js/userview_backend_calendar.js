 ...
 * 
 * @namespace UserviewBackendCalendar
 */
var UserviewBackendCalendar = {
    // :: CONSTANTS
    ...
    initialize: function(defaultEventHandlers) {
        ...
        $('#calendar').fullCalendar({
            ...
            'height': UserviewBackendCalendar.getCalendarHeight(),
            'editable': true,
            'firstDay': 1, // Monday
            ...
            'windowResize': UserviewBackendCalendar.calendarWindowResize,
            'viewDisplay': UserviewBackendCalendar.calendarViewDisplay,
            'dayClick': UserviewBackendCalendar.calendarDayClick,
            'eventClick': UserviewBackendCalendar.calendarEventClick,
            'eventResize': UserviewBackendCalendar.calendarEventResize,
            'eventDrop': UserviewBackendCalendar.calendarEventDrop,
            'eventAfterAllRender': function(view) {
                UserviewBackendCalendar.convertTitlesToHtml();
            }   
        });
        
        // Trigger once to set the proper footer position after calendar 
        // initialization.
        UserviewBackendCalendar.calendarWindowResize();
        ...
        if (GlobalVariables.availableAdminUsers.length > 0) {
            ...
            $.each(GlobalVariables.availableAdminUsers, function(index, adminuser) {
                optgroupHtml += '<option value="' + adminuser['id'] + '" ' +
                        'type="' + UserviewBackendCalendar.FILTER_TYPE_ADMINUSER + '">' +
                        adminuser['subdomain']['subdomain_name'] + '</option>';
            });
            optgroupHtml += '</optgroup>';
            $('#select-filter-item').append(optgroupHtml);
        }      
        /*if (GlobalVariables.availableProviders.length > 0) {
             optgroupHtml = '<optgroup label="' + EALang['providers'] + '" type="providers-group">';
            $.each(GlobalVariables.availableProviders, function(index, provider) {
                var hasGoogleSync = (provider['settings']['google_sync'] === '1') 
                        ? 'true' : 'false';

                optgroupHtml += '<option value="' + provider['id'] + '" '  
                        + 'type="' + UserviewBackendCalendar.FILTER_TYPE_PROVIDER + '" '
        ...
        if (GlobalVariables.availableServices.length > 0) {
            ...
            $.each(GlobalVariables.availableServices, function(index, service) {
                optgroupHtml += '<option value="' + service['id'] + '" ' + 
                        'type="' + UserviewBackendCalendar.FILTER_TYPE_SERVICE + '">' +
                        service['name'] + '</option>';
            });
            optgroupHtml += '</optgroup>';
            $('#select-filter-item').append(optgroupHtml);
        ...
        if (GlobalVariables.user.role_slug == Backend.DB_SLUG_CUSTOMER) {
            $('#select-filter-item optgroup:eq(0)')
                .find('option[value="' + GlobalVariables.subdomain.id_admin + '"]').prop('selected', true);
            console.log('select:', $('#select-filter-item').val());
        ...
        if (defaultEventHandlers === true) {
            UserviewBackendCalendar.bindEventHandlers();
        ...
        if (GlobalVariables.editAppointment != null) {
            ...
            UserviewBackendCalendar.resetAppointmentDialog();
    ...
    bindEventHandlers: function() {
        ...
        $('#select-filter-item').change(function() { 
            UserviewBackendCalendar.refreshCalendarAppointments(
                    $('#calendar'),
            ...
            if ($('#select-filter-item option:selected').attr('type') 
                    === UserviewBackendCalendar.FILTER_TYPE_SERVICE) {
        ...
        $(document).on('click', '.edit-popover', function() {
            ...
            if (UserviewBackendCalendar.lastFocusedEventData.data.is_unavailable == false) {
                var appointment = UserviewBackendCalendar.lastFocusedEventData.data;
                ...
                var unavailable = UserviewBackendCalendar.lastFocusedEventData.data;
                
                // Replace string date values with actual date objects.
                unavailable.start_datetime = GeneralFunctions.clone(UserviewBackendCalendar.lastFocusedEventData.start);
                unavailable.end_datetime = GeneralFunctions.clone(UserviewBackendCalendar.lastFocusedEventData.end);
                
                $dialog = $('#manage-unavailable');
                UserviewBackendCalendar.resetUnavailableDialog();
            ...
            if (UserviewBackendCalendar.lastFocusedEventData.data.is_unavailable == false) {
                ...
                messageButtons['OK'] = function() {
                    ...
                    var postData = { 
                        ...
                        'appointment_id' : UserviewBackendCalendar.lastFocusedEventData.data['id'],
                ...
                var postData = { 
                    'csrfToken': $('#csrftwo').val(),
                    'unavailable_id' : UserviewBackendCalendar.lastFocusedEventData.data.id
                };
        ...
        $('#manage-appointment #save-appointment').click(function() {
            ...
            if (!UserviewBackendCalendar.validateAppointmentForm()) {
            ...
            var appointment = {
                ...
                'id_users_adminuser':$dialog.find('#admin_user').val(),
            ...
            UserviewBackendCalendar.saveAppointment(appointment, customer,
        ...
        $('#manage-unavailable #save-unavailable').click(function() {
            ...
            UserviewBackendCalendar.saveUnavailable(unavailable, successCallback, errorCallback);
        });
            ...
            if ($('#enable-sync').hasClass('enabled') === false) {
                ...
                $.each(GlobalVariables.availableProviders, function(index, provider) {
                    if (provider['id'] == $('#select-filter-item').val()) {
                        ...
                        UserviewBackendCalendar.disableProviderSync(provider['id']);
        ...
        $('.insert-appointment').click(function() {
            ...
            $.each(GlobalVariables.availableAdminUsers, function(index, adminuser) {
                //alert($dialog.find('#select-service').val()+"|"+service['id']);
    ...
    refreshCalendarAppointments: function($calendar, recordId, filterType, startDate, endDate) {
        var postUrl = GlobalVariables.baseUrl + '/index.php/backend_api/ajax_get_userview_calendar_appointments';
        var postData = {
            'csrfToken': $('#csrftwo').val(),
            'record_id': recordId,
        ...
        $.post(postUrl, postData, function(response) {
            ...
            if (filterType === UserviewBackendCalendar.FILTER_TYPE_ADMINUSER && calendarView !== 'month') {
                $.each(GlobalVariables.availableAdminUsers, function(index, adminuser) {
    ...
    calendarEventResize: function(event, dayDelta, minuteDelta, revertFunc, 
        ...
        if (event.data.is_unavailable == false) {
            ...
            UserviewBackendCalendar.saveAppointment(appointment, undefined,
            ...
            UserviewBackendCalendar.saveUnavailable(unavailable, successCallback, undefined);
        }
    },
    ...
    calendarWindowResize: function(view) {
        $('#calendar').fullCalendar('option', 'height', 
                UserviewBackendCalendar.getCalendarHeight());
    },
    ...
    calendarEventClick: function(event, jsEvent, view) {
        ...
        UserviewBackendCalendar.lastFocusedEventData = event;
