 ...
 * 
 * @namespace AdminuserBackendCalendar
 */
var AdminuserBackendCalendar = {
    // :: CONSTANTS
    FILTER_TYPE_PROVIDER: 'provider',
    FILTER_TYPE_SERVICE: 'service',
    FILTER_TYPE_ADMINUSER: 'adminuser',
    ...
    initialize: function(defaultEventHandlers) {
        ...
        $('#calendar').fullCalendar({
            ...
            'height': AdminuserBackendCalendar.getCalendarHeight(),
            'editable': true,
            'firstDay': 1, // Monday
            'slotMinutes': appointmetDuration,
            'snapMinutes': 15,
            'axisFormat': 'HH:mm',
            ...
            'windowResize': AdminuserBackendCalendar.calendarWindowResize,
            'viewDisplay': AdminuserBackendCalendar.calendarViewDisplay,
            'dayClick': AdminuserBackendCalendar.calendarDayClick,
            'eventClick': AdminuserBackendCalendar.calendarEventClick,
            'eventResize': AdminuserBackendCalendar.calendarEventResize,
            'eventDrop': AdminuserBackendCalendar.calendarEventDrop,
            'eventAfterAllRender': function(view) {
                AdminuserBackendCalendar.convertTitlesToHtml();
            }   
        });
        
        // Trigger once to set the proper footer position after calendar 
        // initialization.
        AdminuserBackendCalendar.calendarWindowResize();
        ...
        //                + 'type="' + AdminuserBackendCalendar.FILTER_TYPE_PROVIDER + '" '
        ...
        //                'type="' + AdminuserBackendCalendar.FILTER_TYPE_SERVICE + '">' +
        //                service['name'] + '</option>';
        //    });
        //    optgroupHtml += '</optgroup>';
        //    $('#select-filter-item').append(optgroupHtml);
        //}
        if (GlobalVariables.availableAdminUsers.length > 0) {
            optgroupHtml = '<optgroup label="' + EALang['adminuser'] + '" type="admin-group">';
            $.each(GlobalVariables.availableAdminUsers, function(index, adminuser) {
                var hasGoogleSync = (adminuser['settings']['google_sync'] === '1')
                                ? 'true' : 'false';

                optgroupHtml += '<option value="' + adminuser['id'] + '" ' +
                        'type="' + AdminuserBackendCalendar.FILTER_TYPE_ADMINUSER + '"' +
                        'google-sync="' + hasGoogleSync + '">' +
                        adminuser['subdomain']['subdomain_name']+ '.getbookedin.uk' + '</option>';
            });
            optgroupHtml += '</optgroup>';
            $('#select-filter-item').append(optgroupHtml);
        }
        ...
        //Adminuser check
        if (GlobalVariables.user.role_slug == Backend.DB_SLUG_ADMINUSER) {
            $('#select-filter-item optgroup:eq(0)')
                .find('option[value="' + GlobalVariables.user.id + '"]').prop('selected', true);
            //console.log($('#select-filter-item').val());
        ...
        if (defaultEventHandlers === true) {
            AdminuserBackendCalendar.bindEventHandlers();
        ...
        if (GlobalVariables.editAppointment != null) {
            ...
            AdminuserBackendCalendar.resetAppointmentDialog();
    ...
    bindEventHandlers: function() {
        ...
        $('#select-filter-item').change(function() { 
            AdminuserBackendCalendar.refreshCalendarAppointments(
                    $('#calendar'),
            ...
            if ($('#select-filter-item option:selected').attr('type') 
                    === AdminuserBackendCalendar.FILTER_TYPE_SERVICE) {
        ...
        $(document).on('click', '.edit-popover', function() {
            ...
            if (AdminuserBackendCalendar.lastFocusedEventData.data.is_unavailable == false) {
                var appointment = AdminuserBackendCalendar.lastFocusedEventData.data;
                ...
                var unavailable = AdminuserBackendCalendar.lastFocusedEventData.data;
                
                // Replace string date values with actual date objects.
                unavailable.start_datetime = GeneralFunctions.clone(AdminuserBackendCalendar.lastFocusedEventData.start);
                unavailable.end_datetime = GeneralFunctions.clone(AdminuserBackendCalendar.lastFocusedEventData.end);
                
                $dialog = $('#manage-unavailable');
                AdminuserBackendCalendar.resetUnavailableDialog();
            ...
            if (AdminuserBackendCalendar.lastFocusedEventData.data.is_unavailable == false) {
                ...
                messageButtons['OK'] = function() {
                    ...
                    var postData = { 
                        ...
                        'appointment_id' : AdminuserBackendCalendar.lastFocusedEventData.data['id'],
                ...
                var postData = { 
                    'csrfToken': GlobalVariables.csrfToken,
                    'unavailable_id' : AdminuserBackendCalendar.lastFocusedEventData.data.id
                };
        ...
        $('#manage-appointment #save-appointment').click(function() {
            ...
            if (!AdminuserBackendCalendar.validateAppointmentForm()) {
            ...
            var appointment = {
                ...
                'id_users_adminuser': $dialog.find('#select-adminuser').val(),
            ...
            AdminuserBackendCalendar.saveAppointment(appointment, customer,
        ...
        $('#manage-unavailable #save-unavailable').click(function() {
            ...
            var unavailable = {
                ...
                'id_users_adminuser': GlobalVariables.user.id
                    //$('#select-filter-item').val() // curr provider
            };
            ...
            AdminuserBackendCalendar.saveUnavailable(unavailable, successCallback, errorCallback);
        });
        ...
        $('#enable-sync').click(function() {
            if ($('#enable-sync').hasClass('enabled') === false) {
                ...
                var authUrl = GlobalVariables.baseUrl + '/index.php/google/oauth/'
                        + $('#select-filter-item').val();
                //var authUrl = 'http://localhost/appointment' + '/index.php/google/oauth/'
                //    + $('#select-filter-item').val();
                
                var redirectUrl = GlobalVariables.baseUrl + '/index.php/google/oauth_callback';
                //var redirectUrl = 'http://localhost/appointment' + '/index.php/google/oauth_callback';
                ...
                $.each(GlobalVariables.availableProviders, function(index, provider) {
                    if (provider['id'] == $('#select-filter-item').val()) {
                        ...
                        AdminuserBackendCalendar.disableProviderSync(provider['id']);
        ...
        $('#insert-appointment').click(function() {
            ...
            if ($('#select-filter-item option:selected').attr('type') == 'adminuser') {
                var $providerOption = $dialog.find('#select-adminuser option[value="'
                + $('#select-filter-item').val() + '"]');
            ...
            $.each(GlobalVariables.availableAdminUsers, function(index, adminuser) {
                if (adminuser['id'] == $dialog.find('#select-adminuser').val()) {
                    serviceDuration = adminuser['settings']['duration'];
     ...
     * or FILTER_TYPE_SERVICE or FILTER_TYPE_ADMINUSER.
    ...
    refreshCalendarAppointments: function($calendar, recordId, filterType, startDate, endDate) {
        var postUrl = GlobalVariables.baseUrl + '/index.php/backend_api/ajax_get_adminuser_calendar_appointments';
        var postData = {
            'csrfToken': GlobalVariables.csrfToken,
            'record_id': recordId,
        ...
        $.post(postUrl, postData, function(response) {
            ...
            if (filterType === AdminuserBackendCalendar.FILTER_TYPE_ADMINUSER && calendarView !== 'month') {
                $.each(GlobalVariables.availableAdminUsers, function(index, adminuser) {
                    if (adminuser['id'] == recordId) {
                        var workingPlan = jQuery.parseJSON(adminuser.settings.working_plan);
    ...
    calendarEventResize: function(event, dayDelta, minuteDelta, revertFunc, 
        ...
        if (event.data.is_unavailable == false) {
            ...
            AdminuserBackendCalendar.saveAppointment(appointment, undefined,
            ...
            AdminuserBackendCalendar.saveUnavailable(unavailable, successCallback, undefined);
        }
    },
    ...
    calendarWindowResize: function(view) {
        $('#calendar').fullCalendar('option', 'height', 
                AdminuserBackendCalendar.getCalendarHeight());
    },
    ...
    calendarDayClick: function(date, allDay, jsEvent, view) {
        if (allDay) {
            ...
            //alert('Click on slot:',date.toString());
            console.log(date);
    ...
    calendarEventClick: function(event, jsEvent, view) {
        ...
        if ($parent.hasClass('fc-unavailable') || $altParent.hasClass('fc-unavailable')) {
            ...
            html = 
                    ...
                    '<strong>' + EALang['provider'] + '</strong> ' 
                        + event.data['admnuser']['first_name'] + ' '
                        + event.data['admnuser']['last_name']
                        + '<br>' +
        ...
        AdminuserBackendCalendar.lastFocusedEventData = event;
    ...
    resetAppointmentDialog: function() {
        ...
        $dialog.find('#select-adminuser').empty();
        $.each(GlobalVariables.availableAdminUsers, function(index, adminuser) {
            var canProvideService = true;

            $.each(adminuser['services'], function(index, serviceId) {
            ...
            if (canProvideService) { // Add the provider to the listbox.
                var option = new Option(adminuser['first_name']
                + ' ' + adminuser['last_name'], adminuser['id']);
                $dialog.find('#select-adminuser').append(option);
            }

            $dialog.find('#select-adminuser')
                .find('option[value="' + GlobalVariables.user.id + '"]').prop('selected', true);
            //console.log($('#select-filter-item').val());
            $dialog.find('#select-adminuser').prop('disabled', true);
        });
        //$.each(GlobalVariables.availableProviders, function(index, provider) {
        ...
        //Code to open datetime picker for start-datetime box
        ...
        //Code to open datetime picker for end-datetime box
    ...
    validateAppointmentForm: function() {
        ...
        try {
            ...
            now = Date.parseExact(now, 'dd/MM/yyyy HH:mm');
            ...
            if (start > end) {
                ...
                $dialog.find('#start-datetime').parents().eq(1).addClass('error');
                $dialog.find('#end-datetime').parents().eq(1).addClass('error');
                throw EALang['start_end_date_before_current_error'];
            }
