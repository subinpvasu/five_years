 ...
 */
ProviderHelper.prototype.bindEventHandlers = function() {
    /**
    ...
    $('#filter-providers form').submit(function(event) {
        ...
        BackendUser.helper.resetForm();
        BackendUser.helper.filter(key);
        return false;
    });
    ...
    $(document).on('click', '.provider-row', function() {
        ...
        var provider = {};
        $.each(BackendUser.helper.filterResults, function(index, item) {
        ...
        BackendUser.helper.display(provider);
    ...
    $('#add-provider').click(function() {
        ...
        BackendUser.wp.setup(GlobalVariables.workingPlan);
        BackendUser.wp.timepickers(false);
    });
    ...
    $('#delete-provider').click(function() {
        ...
        messageBtns[EALang['delete']] = function() {
            BackendUser.helper.delete(providerId);
    ...
    $('#save-provider').click(function() {
        var provider = {
            ...
            'settings': {
                ...
                'working_plan': JSON.stringify(BackendUser.wp.get()),
        ...
        if (!BackendUser.helper.validate(provider)) return;

        BackendUser.helper.save(provider);
    });
    ...
    $('#cancel-provider').click(function() {
        ...
        BackendUser.helper.resetForm();
        if (id != '') {
            BackendUser.helper.select(id, true);
        }
    });
 ...
 */
ProviderHelper.prototype.save = function(provider) {
    //////////////////////////////////////////////////
    ...
    $.post(postUrl, postData, function(response) {
        ...
        /*
        $('#filter-providers .key').val('');
        BackendUser.helper.filter('', response.id, true);
        */
    }, 'json');
};
 ...
 */
ProviderHelper.prototype.delete = function(id) {
    ...
    $.post(postUrl, postData, function(response) {
        ...
        BackendUser.helper.resetForm();
        BackendUser.helper.filter($('#filter-providers .key').val());
    }, 'json');
};
 ...
 */
ProviderHelper.prototype.validate = function(provider) {
    ...
    try {
        ...
        if ($('#provider-password').val().length < BackendUser.MIN_PASSWORD_LENGTH
                && $('#provider-password').val() != '') {
            $('#provider-password, #provider-password-confirm').css('border', '2px solid red');
            throw EALang['password_length_notice'].replace('$number', BackendUser.MIN_PASSWORD_LENGTH);
        }
 ...
 */
ProviderHelper.prototype.resetForm = function() {
    $('#filter-providers .selected-row').removeClass('selected-row');
    ...
    BackendUser.wp.timepickers(true);
 ...
 */
ProviderHelper.prototype.display = function(provider) {
    ...
    BackendUser.wp.setup(workingPlan);
    $('.breaks').find('.edit-break, .delete-break').prop('disabled', true);
};
 ...
 * displayed.
 */
ProviderHelper.prototype.filter = function(key, selectId, display) {
    ...
    $.post(postUrl, postData, function(response) {
        ...
        if (selectId != undefined) {
            BackendUser.helper.select(selectId, display);
        }
    }, 'json');
};
 ...
 */
ProviderHelper.prototype.getFilterHtml = function(provider) {
 ...
 */
ProviderHelper.prototype.editableBreakDay = function($selector) {
    var weekDays = {};
    ...
    $selector.editable(function(value, settings) {
        ...
        'onreset': function(settings, td) {
            if (!BackendUser.enableCancel) return false; // disable ESC button
        },
        'onsubmit': function(settings, td) {
            if (!BackendUser.enableSubmit) return false; // disable Enter button
        }
    });
};
 ...
 * @param {object} $selector The cells to be initialized.
 */        
ProviderHelper.prototype.editableBreakTime = function($selector) {
    $selector.editable(function(value, settings) {
 ...
 */
ProviderHelper.prototype.select = function(id, display) {
    if (display == undefined) display = false;
    ...
    if (display) {
        $.each(BackendUser.helper.filterResults, function(index, provider) {
            if (provider.id == id) {
                BackendUser.helper.display(provider);
                $('#edit-provider, #delete-provider').prop('disabled', false);
                return false;
