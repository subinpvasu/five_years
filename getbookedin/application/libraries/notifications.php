...
class Notifications {
    ...
    public function send_appointment_details($appointment_data, $provider_data, $service_data, 
        ...
        $replace_array = array(
            ...
            '$appointment_start_date'   => date('D, M j, Y \a\t G:i a', strtotime($appointment_data['start_datetime'])),
            '$appointment_end_date'     => date('D, M j, Y \a\t G:i a', strtotime($appointment_data['end_datetime'])),
    ...
    public function send_delete_appointment($appointment_data, $provider_data, 
        ...
        $replace_array = array(
            ...
            '$appointment_service'  => $service_data['name'],
            '$appointment_provider' => $provider_data['first_name'] . ' ' . $provider_data['last_name'],
            '$appointment_date'     => date('D, M j, Y \a\t G:i a', strtotime($appointment_data['start_datetime'])),
    ...
    public function send_new_installation($company_name, $company_email, $company_link) {
        ...
        $mail->AddAddress('info@vbridge.co.uk');
        $mail->IsHTML(true);
        $mail->CharSet = 'UTF-8';
        $mail->Subject = 'New vbridge!Appointments Installation';
