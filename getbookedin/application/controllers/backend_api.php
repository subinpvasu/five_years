...
//            $provider = $this->providers_model->get_row($appointment['id_users_provider']);
            $adminuser = $this->admin_model->get_row($appointment['id_users_adminuser']);
...
//                            ->get_setting('notifications', $provider['id']);
                $send_adminuser = $this->admin_model
                    ->get_settings_name('notifications', $adminuser['id']);
                ...
                $this->notifications->send_appointment_details($appointment, $adminuser,
                        $service, $customer, $company_settings, $customer_title, 
                        $customer_message, $customer_link, $customer['email']);

                if ($send_adminuser == TRUE) {
                    $this->notifications->send_appointment_details($appointment, $adminuser,
                            $service, $customer, $company_settings, $provider_title, 
                            $provider_message, $provider_link, $adminuser['email']);
                }
...
//                            ->get_setting('notifications', $provider['id']);
                ...
                if ($send_adminuser == TRUE) {
                    $this->notifications->send_delete_appointment($appointment, $adminuser,
                            $service, $customer, $company_settings, $adminuser['email'],
                            $_POST['delete_reason']);
                }
    	...
    	try {
            ...
            foreach($customers as &$customer) {
                $appointments = $this->appointments_model
                        ->get_batch(array('id_users_customer' => $customer['id'], 'id_users_adminuser' => $admin_id));
...
//                            ->get_row($appointment['id_users_provider']);
                        $appointment['adminuser'] = $this->admin_model
                            ->get_row($appointment['id_users_adminuser']);
                    }
...
//            $adminuser = $this->admin_model->get_row($unavailable['id_users_adminuser']);
...
//            }
    ...
    public function ajax_filter_adminuser() {
        try {
            if ($this->privileges[PRIV_USERS]['view'] == FALSE) {
                throw new Exception('You do not have the required privileges for this task.');
            }
            ...
            $where =
                ...
                'OR city LIKE "%' . $key . '%" OR state LIKE "%' . $key . '%" ' .
                'OR zip_code LIKE "%' . $key . '%" OR notes LIKE "%' . $key . '%")';
            $adminuser = $this->admin_model->get_batch($where);
    ...
    public function ajax_save_settings() {
        try {
            ...
            if ($_POST['type'] == SETTINGS_SYSTEM) {
                ...
                //$this->header_model->save_settings(json_encode($header, true));
                //echo $re;
            }
    ...
    public function ajax_save_profile_header_settings() {
        ...
        $settings = array(
            ...
            'settings' =>  array(
                ...
                'notifications' => $this->input->post('user-notifications')
            )
        );
        ...
        $header = array(
            'id_admin' => $this->input->post('user-id'),
...
//                $this->load->view('adminuser/no_image', $error);

                $this->load->model('admin_model');
                $this->load->model('header_model');

//            $header['logo'] = $data['upload_data']['file_name'];

                $this->admin_model->save_profile_settings($settings);
                $this->header_model->save_settings($header);

                $this->load->library('../controllers/adminuser');
...
//            $this->load->view('upload_success', $data);
    ...
    public function ajax_save_profile_settings() {
        try {
            ...
            if ($_POST['type'] == SETTINGS_USER) {
                ...
                $header = json_decode($_POST['settings'],true);
                //print_r($_POST['settings']->header);
                //print_r($header['header']);
//                print_r($_FILES['uploadBtn']);
//                exit();
                $this->load->model('admin_model');
                $this->load->model('header_model');

                $this->admin_model->save_profile_settings(json_decode($_POST['settings'], true));
                $this->header_model->save_settings($header['header']);
                //echo $re;
    ...
    public function ajax_save_header_settings() {
        try {

            $this->load->model('header_model');

            $this->header_model->save_settings(json_decode($_POST['header'], true));
            echo json_encode(AJAX_SUCCESS);
    ...
    public function ajax_validate_email() {
        try {
            ...
            // for the rest user types for now (providers, secretaries).
            $this->load->model('admin_model');
            $is_valid = $this->admin_model->validate_useremail($_POST['username'], $_POST['user_id']);
            echo json_encode($is_valid);
        } catch(Exception $exc) {
            echo json_encode(array(
                'exceptions' => array(exceptionToJavaScript($exc))
...
//            $google_sync = $this->providers_model->get_setting('google_sync', $_POST['provider_id']);
            $google_sync = $this->admin_model->get_settings_name('google_sync', $_POST['provider_id']);
            if ($google_sync) {
                $google_token = json_decode($this->admin_model->get_settings_name('google_token', $_POST['provider_id']));
                $this->google_sync->refresh_token($google_token->refresh_token);
    ...
    /**
     * custom created
     * [AJAX] Save (insert or update) a User record into database.
     *
     * @param array $_POST['user'] A json encoded array that contains the user data. If an 'id'
     * value is provided then the record is going to be updated.
     * @return string Returns the success contant 'AJAX_SUCCESS' so javascript knows that
     * everything completed successfully.
     */
    public function ajax_save_user() {
        try {
            ...
            if (!isset($user['settings']['working_plan'])) {
                $this->load->model('settings_model');
                $user['settings']['working_plan'] = $this->settings_model
                    ->get_setting('company_working_plan');
            }

            $user_id = $this->providers_model->add($user);
...
//            }
            ...
            if (!isset($admin['settings']['working_plan'])) {
                $this->load->model('settings_model');
                $admin['settings']['working_plan'] = $this->settings_model
                    ->get_setting('company_working_plan');
            }

            $admin_id = $this->admin_model->add($admin);

            $this->session->set_userdata(array(
                'status' => AJAX_SUCCESS,
                'id' => $admin_id
            )); // Save data on user's session.
    ...
    public function ajax_save_subdomain() {
        try {
            ...
            $domain = json_encode($_POST['subdomain'], true);
    ...
    public function ajax_get_adminuser_calendar_appointments() {
        try {
            if ($this->privileges[PRIV_APPOINTMENTS]['view'] == FALSE) {
                throw new Exception('You do not have the required privileges for this task.');
            }
            ...
            if ($_POST['filter_type'] == FILTER_TYPE_PROVIDER) {
                $where_id = 'id_users_provider';
            } elseif ($_POST['filter_type'] == FILTER_TYPE_ADMINUSER) {
                $where_id = 'id_users_adminuser';
            } else {
                $where_id = 'id_services';
            }
            ...
            foreach($response['appointments'] as &$appointment) {
                ...
                $appointment['admnuser'] = $this->admin_model->get_row($appointment['id_users_adminuser']);
            ...
            // Get unavailable periods (only for adminuser).
            if ($_POST['filter_type'] == FILTER_TYPE_ADMINUSER) {
                $where_clause = array(
                    $where_id => $_POST['record_id'],
                    //'start_datetime >=' => $_POST['start_date'],
                    //'end_datetime <=' => $_POST['end_date'],
...
//            }
    ...
    public function ajax_get_userview_calendar_appointments() {
        try {
            if ($this->privileges[PRIV_APPOINTMENTS]['view'] == FALSE) {
                //throw new Exception('You do not have the required privileges for this task.');
                $this->privileges[PRIV_APPOINTMENTS]['view'] = TRUE;
            }
            ...
            if ($_POST['filter_type'] == FILTER_TYPE_PROVIDER) {
                $where_id = 'id_users_provider';
            } else if($_POST['filter_type'] == FILTER_TYPE_ADMINUSER) {
                $where_id = 'id_users_adminuser';
            }         else
            {
                $where_id = 'id_services';
            ...
            $where_clause = array(
                $where_id => $_POST['record_id'],
                'id_users_customer'=>  $this->session->userdata('who'),
                'start_datetime >=' => $_POST['start_date'],
                'end_datetime <=' => $_POST['end_date'],
                'is_unavailable' => FALSE
            );
