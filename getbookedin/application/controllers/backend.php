...
class Backend extends CI_Controller {
    ...
    /**
     * Display the backend user page(individual user info).
     *
     * In this page the provider user will be able to manage his/her own account.
     * By this, we mean only the provider. This is also
     * the page where the provider defines working plans for appointment.
     */
    public function user() {
        $this->session->set_userdata('dest_url', $this->config->item('base_url'). '/index.php/backend/user');
        if(!$this->hasPrivileges(PRIV_USERS)) return;
    ...
    /**
     * Custom created to call private function hasPrivileges()
     */
    public function checkPrivileges($page) {
        return $this->hasPrivileges($page);
    }
