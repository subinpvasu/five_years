...
//                        $_SESSION['oauth_provider_id']);
                ...
                $this->admin_model->set_setting('google_sync', TRUE,
                    $_SESSION['oauth_provider_id']);
                $this->admin_model->set_setting('google_token', $token,
                    $_SESSION['oauth_provider_id']);
                $this->admin_model->set_setting('google_calendar', 'primary',
                    $_SESSION['oauth_provider_id']);
            } else {
                echo '<h1>Sync AdminUser id not specified!</h1>';
            }
       	} else {
...
//            $provider = $this->providers_model->get_row($provider_id);
            $adminuser = $this->admin_model->get_row($provider_id);
...
//            $google_sync = $this->providers_model->get_setting('google_sync', $provider['id']);
            $google_sync = $this->admin_model->get_settings_name('google_sync', $adminuser['id']);
            if (!$google_sync) {
...
//            $google_token = json_decode($this->providers_model->get_setting('google_token', $provider['id']));
            $google_token = json_decode($this->admin_model->get_settings_name('google_token', $adminuser['id']));
            $this->load->library('google_sync');
...
//            $sync_future_days = $this->providers_model->get_setting('sync_future_days', $provider['id']);
            ...
            $sync_past_days = $this->admin_model->get_settings_name('sync_past_days', $adminuser['id']);
            $sync_future_days = $this->admin_model->get_settings_name('sync_future_days', $adminuser['id']);
...
//                'id_users_provider' => $provider['id']
                'id_users_adminuser' => $adminuser['id']
            );
            ...
            foreach($appointments as $appointment) {
                ...
                if ($appointment['id_google_calendar'] == NULL) {
                    $google_event = $this->google_sync->add_appointment($appointment, $adminuser,
                    ...
                    try {
                        $google_event = $this->google_sync->get_event($adminuser, $appointment['id_google_calendar']);
                        
                        if ($google_event->status == 'cancelled') {
                            throw new Exception('Event is cancelled, remove the record from Easy!Appointments.');
                        }
                        
                        // If gcal event is different from appointment then update e!a record.
                        $is_different = FALSE;
            ...
            $google_calendar = $adminuser['settings']['google_calendar'];
            ...
            foreach($events->getItems() as $event) {
                ...
                if (count($results) == 0) {
                    ...
                    $appointment = array(
                        ...
                        'id_users_adminuser' => $provider_id,
