...
//        );
        ...
        // Validate adminuser record
        $where_clause = array(
            'id' => $unavailable['id_users_adminuser'],
            'id_roles' => $this->db->get_where('ea_roles', array('slug' => DB_SLUG_ADMIN_USER))->row()->id
        );
        
        if ($this->db->get_where('ea_admin', $where_clause)->num_rows() == 0) {
            throw new Exception('Adminuser id was not found in database.');
        }
     ...
     * 
     */
    public function select_appointments_notification($date)
    {
        $sql = "SELECT * FROM ea_appointments WHERE date(book_datetime)='.$date.'";
