						...
						else 
						{
							...
							foreach($songs_ist as $single_song)
							{
								...
								for($temp_i=0;$temp_i<count($tag_order);$temp_i++)
								{
									...
									{
										$temp_sorted_songs[$temp_i][] = $single_song;
										break;
									}
								}
								//exit;
							...
							if($songs_in_current_playlist != 0)
							{
								if(is_array($scheduled_songs) && !empty($scheduled_songs))							
								{
									if(!empty($scheduled_songs[0]))
									{
										foreach($scheduled_songs[0] as $scheduled_song) 
										{
											if(!in_array($scheduled_song,$now_added)) $now_added[] = $scheduled_song;
										}
									}
									if(count($scheduled_songs == 2) && !empty($scheduled_songs[1]))
									{
										foreach($scheduled_songs[1] as $key3=>$scheduled_song_time) 
										{
											$now_added_song_time[$key3] = $scheduled_song_time;
										}
									}
									
								}
								if($scheduled_songs[2] < $scheduled_songs[3])
								{
									$scheduled_songs = $this->print_schedule($id,$temp_sorted_songs,$now_added,$last_played_songs,1,$scheduled_songs[2],$filename,$time_slot,$file,$now_added_song_time,$ads_and_jing,0,$tested_index);
									//echo 'Order 2';
									//print_r($scheduled_songs);
									if(is_array($scheduled_songs) && !empty($scheduled_songs))							
									{
										if(!empty($scheduled_songs[0]))
										{
								...
								if($scheduled_songs[2] < $scheduled_songs[3])
								{
									...
									{
										//echo 'Order 3';
										$scheduled_songs = $this->print_schedule($id,$temp_sorted_songs,$now_added,$last_played_songs,2,$scheduled_songs[2],$filename,$time_slot,$file,$now_added_song_time,$ads_and_jing,0,$tested_index);
										if(is_array($scheduled_songs) && !empty($scheduled_songs))							
										{
											if(!empty($scheduled_songs[0]))
											{
											...
											if($scheduled_songs[2] < $scheduled_songs[3])
											{
												//echo $scheduled_songs[2].'<'.$scheduled_songs[3];
												goto a;
