...
group by date_format(transaction_date,'%Y'),date_format(transaction_date,'%m')";
    ...
    else
    {
         $query = $this->db->query ( "SELECT *,DATE_FORMAT(SCHEDULE_DATE, '%m/%d/%Y') AS SCHEDULE_DATE FROM work_order_parts INNER
         JOIN work_order ON work_order_parts.PK_WO = work_order.PK_WO WHERE
         work_order.PK_WO='$pkwork' AND work_order_parts.WORK_VALUE>0 AND work_order_parts.WORK_TYPE !=  'DINGHY' ORDER BY work_order_parts.PK_WO_PARTS ASC " );
          return $query->result ();
    }
    ...
    public function get_clean_work_order_info_pkcustomer_changed($pkcustomer,$season) {
