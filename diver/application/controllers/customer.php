...
//             exit();
               ...
               //print_r($data['scheduledate']);exit();
                ...
                $current_year = date('Y',strtotime("$schedule"));
					$current_mon = date('m',strtotime("$schedule"));
					if($current_mon == 10) 
					{
					...
					{
//						echo $data['customer'][0]->PK_CUSTOMER;
						$new_date = date('Y-m-d',strtotime('+2 weeks', strtotime($schedule)));
                                                $season = $this->what_season_present_new($new_date);
//						echo $this->what_season_present_new($new_date);
//                                                exit();
						$new_season_wo_info = $this->customer_model->get_clean_work_order_info_pkcustomer_changed($data['customer'][0]->PK_CUSTOMER,$season);
						//echo '<pre>';
...
//                                                exit();
						if(!empty($new_season_wo_info))
						{
							foreach($new_season_wo_info as $selected_season_wo_info)
							{
								...
								{
									$data['newdate'] = $this->calculateScheduleDate($schedule, 'MONTHLY ONLY');
									//echo $data['newdate'];
								}
