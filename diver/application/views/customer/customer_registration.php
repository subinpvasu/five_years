...
<select name="bill_mode" class="select" id="bill_mode">
		...
		foreach ( $bill as $row ) {
			if (ucwords(strtolower($row->OPTIONS)) === $customers->DELIVERY_MODE) {
				?>
 			 	 	<option selected value="<?php echo ucwords(strtolower($row->OPTIONS)); ?>"><?php echo $row->OPTIONS; ?></option>
 			 	 	<?php
			} else {
				?>
		...
		echo form_label ( 'Fax', 'referred_fax' ) . form_input ( array ('class' => 'textbox', 'name' => 'referred_fax', 'value' => $customers->ALT_FAX_NO, ' class="textbox" ' ) ) . br ();
...
<?php
	...
	echo form_label ( 'Fax', 'referred_fax' ) . form_input ( array ('class' => 'textbox', 'name' => 'referred_fax','value' =>  $this->session->userdata('referred_fax') ) ) . br ();
	echo form_label ( 'Status', 'status' ) ;
	/* 
	...
	<div
		style="width: 100%; float: left; margin-top: 15px; text-align: center; padding-left: 100px;">
 	 	<?php //<a href="'.base_url().'index.php/customer/customer_vessel/" class="buttonlink">Next</a>
