...
function onscreenSearch(str)
{
	...
	xmlhttp.open("POST","/btwdive/index.php/customer/screenSearchResults/"+str,true);
	xmlhttp.send();
...
function showDetailBox(str)
{
	if(document.getElementById(str).checked==true)
	{
		...
		xmlhttp.open("POST","/btwdive/index.php/customer/getAnodeDetails/"+str,true);
		xmlhttp.send();
	}
}
...
function addAnode(anode,anodeid)
{
	...
	var anodedate = document.getElementById("datepickeranode").value;
	...
	$(document).ready(function(){
		   ...
		   $.ajax({url:"/btwdive/index.php/customer/updateAnodeTable/",
				type:"post",
				 data: {
					 anodes : newanodeid,
					 type:ftype,
...
function editAnodeFromVessel(str)
{
	...
	xmlhttp.open("POST","/btwdive/index.php/customer/getAnodeDetailsFromVessel/"+str,true);
	xmlhttp.send();
...
function editAddedAnode(str)
{
	...
	if(anode.length>5)
	{
		var boxTable = '<table ><tr><td colspan="3" style="text-align:center;">Add Anode Types to Vessel</td></tr>			<tr><td>Anode Type</td><td colspan="2"><input type="text" class="textbox" id="anodetype" value="'+anode[1]+'"/></td></tr>			<tr><td>Description</td><td colspan="2"><input type="text" class="textbox" id="description" value="'+anode[2]+'"/></td></tr>			<tr><td>Schedule Change</td><td colspan="2"><input type="text" class="textbox" id="schedule" value="'+anode[4]+' Months"/></td></tr>			<tr><td>No of Anodes</td><td colspan="2"><input id="anodenum" type="text" class="textbox" id="" value="1"/></td></tr>			<tr><td>List Price</td><td colspan="2"><input type="text" class="textbox" id="listprice" value="'+anode[3]+'"/></td></tr>			<tr><td>Discount %</td><td colspan="2"><input type="text" class="textbox" id="discount" value="'+anode[6]+'" disabled/></td></tr>			<tr><td>Discount Price</td><td colspan="2"><input type="text" class="textbox" id="disprice" value="'+anode[5]+'"/></td></tr>			<tr><td>Anode Last</td><td colspan="2"><input type="text" class="textbox" id="datepickeranode" value="'+anode[7]+'" /></td></tr>	</table><button class="btn"  onclick="modifyAnode('+anode[0]+')">Modify</button><button class="btn"  onclick="donotaddAnode('+anode[0]+')">Cancel</button>';		
	...
	var dis   = document.getElementById("anode_discount").value;
	var disprice = eval(parseFloat(anode[3])-parseFloat(anode[3])*parseFloat(dis)/100);
	var boxTable = '<table><tr><td colspan="3" style="text-align:center;">Add Anode Types to Vessel</td></tr>		<tr><td>Anode Type</td><td colspan="2"><input type="text" class="textbox" id="anodetype" value="'+anode[1]+'"/></td></tr>		<tr><td>Description</td><td colspan="2"><input type="text" class="textbox" id="description" value="'+anode[2]+'"/></td></tr>		<tr><td>Schedule Change</td><td colspan="2"><input type="text" class="textbox" id="schedule" value="'+anode[4]+' Months"/> </td></tr>		<tr><td>No of Anodes</td><td colspan="2"><input type="text" id="anodenum"  class="textbox" value="1"/></td></tr>		<tr><td>List Price</td><td colspan="2"><input type="text" class="textbox" id="listprice" value="'+anode[3]+'"/></td></tr>		<tr><td>Discount %</td><td colspan="2"><input type="text" class="textbox" value="'+dis+'" id="discount" disabled/></td></tr>		<tr><td>Discount Price</td><td colspan="2"><input type="text" class="textbox" id="disprice" value="'+eval((disprice).toFixed(2))+'"/></td></tr>		<tr><td>Anode Last</td><td colspan="2"><input type="text" class="textbox" id="datepickeranode" /></td></tr>	</table><button onclick="modifyAnode('+anode[0]+')" class="btn" >Modify</button><button class="btn"  onclick="donotaddAnode('+anode[0]+')">Cancel</button>';
...
if(date =='')
{
	alert ("Try Again with Anode Last");
	return false;
	 
}
else
{
	 $.ajax({url:"/btwdive/index.php/customer/updateAnodeTableEdited/",
			type:"post",
			 data: {
...
$(document).ready(function(){
	  $("tr").click(function(){
	   ...
	   $.ajax({url:"/btwdive/index.php/customer/getPrimaryKeyAnode/",
			type:"post",
			 data: {anodes : anode},
		success:function(result){
			alert(result); }});
...
function removeAnodeAdded(str)
{
	...
	xmlhttp.open("POST","/btwdive/index.php/customer/remove_anode_added/"+str,true);
	xmlhttp.send();
	}
}
...
if (isset ( $customer )) {
    ...
    foreach ( $customer as $customers ) :
        ...
        $row = $row .
                                         '<tr style="cursor:pointer;" onclick="editAnodeFromVessel(' .
                                         $customers->PK_VESSEL_ANODE .
                                         ')"><td><input type="checkbox" name="listed_anodes[]" value="' .
                                         $customers->PK_VESSEL_ANODE .
                                         '"  checked style="width:10px;" onclick="removeAnodeAdded(' .
                                         $customers->PK_VESSEL_ANODE .
                                         ')"/>' .
                                         $customers->ANODE_TYPE .
                                         '</td><td>' .
                                         $customers->LIST_PRICE .
                                         '</td><td>' .
                                         $customers->DISCOUNT_PRICE .
                                         '</td></tr>';
    
    endforeach
    ;
    echo '<input type="hidden" name="vessel_anode_id" id="vessel_anode_id" value="' .
                                     $this->session->userdata ( 'anode_number' ) .
                                     '"/>';
echo form_label ( 'Discount %', 'discount' ) . form_input ( 'discount', $discount, 'id="anode_discount" class="textbox"' ) . br ();
echo form_label ( 
    'Search Anodes to be Added', 
    'add_anodes' ) .
     form_input ( 
                                    'anodes', 
                                    '', 
                                    'id="anodes" class="textbox" onkeyup="onscreenSearch(this.value)" onfocus="onscreenSearch(this.value)"' ) .
     br ();
...
echo '<a href="' .
 base_url () .
 'index.php/customer/customer_services/' .
 $this->session->userdata ( 'customerid' ) .
 '" class="buttonlink">Back</a> <a href="' .
 base_url () .
 'index.php/customer/customer_misc/' .
 $this->session->userdata ( 'customerid' ) .
 '" class="buttonlink">Next</a> <input type="submit" name="submit" style="margin-top:5px;height:36px;" value="Save" class="btn"/> <a href="' .
 base_url () .
...
} else if (isset ( $session_customer )) {
$discount = 0.00;
$row = '';
...
echo form_label ( 'Discount %', 'discount' ) . form_input ( 'discount', $discount, 'id="anode_discount" class="textbox"' ) . br ();
echo form_label ( 'Search Anodes to be Added', 'add_anodes' ) . form_input ( 
'anodes', 
'', 
'id="anodes" class="textbox" onkeyup="onscreenSearch(this.value)" onfocus="onscreenSearch(this.value)"' ) . br ();
...
echo '<a href="' .
 ...
 'index.php/customer/customer_services/' .
 $this->session->userdata ( 'customerid' ) .
 '" class="buttonlink">Back</a> <input type="submit" name="submit" style="margin-top:5px;height:36px;" value="Next" class="btn"/> <input type="submit" name="submit" style="margin-top:5px;height:36px;" value="Save" class="btn"/> <a href="' .
 base_url () .
 'index.php/customer/customer_session/" id="exit" class="buttonlink">Exit</a>';
?></div>
				<?php
...
echo '<a href="' .
 ...
 'index.php/customer/customer_services/' .
 $this->session->userdata ( 'customerid' ) .
 '" class="buttonlink">Back</a> 
	 	 	 <input type="submit" name="submit" style="margin-top:5px;height:36px;" value="Next" class="btn"/>
	 	 	
	 	 	<input type="submit" name="submit" style="margin-top:5px;height:36px;" value="Save" class="btn"/> <a href="' .
 base_url () .
 'index.php/customer/customer_session/" id="exit" class="buttonlink">Exit</a>';
?></div>
		<?php
